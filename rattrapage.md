...
Quel langage est utilisé ? Golang version 1.22 comme décrit dans go.mod
...
Quel langage ? => Golang version 1.22 comme décrit dans go.mod
Point d'entrée du programme ? =>main.go avec la fonction main()
Port utilisé ? => 8080 (comme vue dans main.go "r.Run(":8080")")
comment installer les dépendances ? => go mod download -> go.mod ->  le module utilise github.com/gin-gonic/gin v1.10.0
...
Build : j'ai utilisé docker build -t go-api .

Run : docker run -p 8080:8080 go-api
